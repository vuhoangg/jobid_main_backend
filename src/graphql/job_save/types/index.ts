import {
  GraphQLInputObjectType,
  GraphQLList,
  GraphQLNonNull,
  GraphQLObjectType,
  GraphQLString,
} from "graphql";
import {PageInfo} from "../../types";
import {User} from "../../user/types";
import {JobPost} from "../../job_post/types";

export const JobSave = new GraphQLObjectType({
  description: "Represents a job save.",
  fields: {
    _id: {type: new GraphQLNonNull(GraphQLString)},
    job_post: {type: JobPost},
    user: {type: User},
    created_at: {type: new GraphQLNonNull(GraphQLString)},
    updated_at: {type: new GraphQLNonNull(GraphQLString)},
  },
  name: "JobSave",
});
export const JobSaveEdge = new GraphQLObjectType({
  description: "A list of edges.",
  fields: {
    cursor: {type: new GraphQLNonNull(GraphQLString)},
    node: {
      description: "The item at the end of JobSaveEdge.",
      resolve: (parent) => parent.node,
      type: new GraphQLNonNull(JobSave),
    },
  },
  name: "JobSaveEdge",
});
export const JobSaveConnection = new GraphQLObjectType({
  description: "List of job saves.",
  fields: {
    edges: {
      resolve: (parent) => parent.edges,
      type: new GraphQLNonNull(new GraphQLList(JobSaveEdge)),
    },
    pageInfo: {type: new GraphQLNonNull(PageInfo)},
  },
  name: "JobSaveConnection",
});

export const JobSaveInput = new GraphQLInputObjectType({
  fields: {
    job_post: {type: new GraphQLNonNull(GraphQLString)},
  },
  name: "JobSaveInput",
  description: "The updated properties for a job save.",
});

export const JobSaveArguments = {
  _id: {type: new GraphQLNonNull(GraphQLString)},
};
